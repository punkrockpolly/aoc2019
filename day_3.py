...
def crossed_wires(puzzle_input):
    (wire_a, wire_b) = puzzle_input

    path_a = make_moves(wire_a)
    path_b = make_moves(wire_b)

    matches = intersection(path_a, path_b)
    distances = sorted([abs(match[0]) + abs(match[1]) for match in matches])

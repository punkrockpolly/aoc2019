...
def intcode(puzzle_data, n=0, relative_base=0):
    puzzle_data.extend([0 for _ in range(10000)])
...
def analyze_view(puzzle_data):
    droid = intcode(puzzle_data)
    parsed_string = ''
    while True:
        output = next(droid)
        while output:
            parsed_string += chr(output)
            try:
                output = next(droid)
...
def parse_map(map_input):
    ...
    for y, line in enumerate(map_input.splitlines()):
        for x, char in enumerate(line):
            if char == '#':
                scaffolding[(x, y)] = 'PATH'
            elif char == '.':
                scaffolding[(x, y)] = 'SPACE'
            else:
                scaffolding[(x, y)] = char
                robot_start = (x, y)
    return scaffolding, robot_start
...
def try_move(robot_position, direction):
    return (robot_position[0] + direction[0], robot_position[1] + direction[1])


def find_path(scaffolding_map, robot_position):
    directions = {'^': (0, -1), '>': (1, 0), 'v': (0, 1), '<': (-1, 0)}
    path = []
    prev_xy = None
    turns = 0
    forward = 0
    robot = scaffolding_map[robot_position]
    direction = directions[robot]
    while True:
        new_xy = try_move(robot_position, direction)
        while new_xy == prev_xy or scaffolding_map.get(new_xy) != 'PATH':
            robot = rotate(robot)
            direction = directions[robot]
            turns += 1
            if turns > 4:  # no more places to go
                path.append(forward)
                return path
            new_xy = try_move(robot_position, direction)
        if turns == 0:
            ...
            forward = 1
            path.append('L')
        prev_xy = robot_position
        robot_position = new_xy
        turns = 0


def move_robot(puzzle_data):
    puzzle_data[0] = 2
    droid = intcode(puzzle_data)
    commands = ['A,B,A,C,B,C,B,C,A,C',
                'R,12,L,6,R,12',
                'L,8,L,6,L,10',
                'R,12,L,10,L,6,R,10',
                'n',
...
def vacuum_robot(puzzle_data):
    data_copy = puzzle_data.copy()
    scaffolding_view = analyze_view(puzzle_data)
    scaffolding_map, robot_position = parse_map(scaffolding_view)
    path = find_path(scaffolding_map, robot_position)
    path = ','.join(str(s) for s in path[1:])
    print(path)
    print(move_robot(data_copy))

vacuum_robot(DAY_17_INPUT)

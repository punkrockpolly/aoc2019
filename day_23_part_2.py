...
def intcode_network(puzzle_data):
        ...
        for addr, computer in network.items():
            ...
            while resp:
                ...
                if resp == 255:
                    ...
                    msg_q[resp].append(x)
                    msg_q[resp].append(y)
                resp = computer.send(-1)  # continuously trying to receive packets
        if i - last_msg > 100:  # no packets were received in 100 iterations
            x, y = nat
            if y == prev_y:
                return y
            msg_q[0].append(x)
